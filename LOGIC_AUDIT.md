...
| --- | --- | --- | --- | --- |
| R1 | Resolved | `worker/leaderboard.js` | The `/optin` endpoint formerly only checked that the `X-Telegram-Init-Data` header was *present*, never verifying the HMAC nor that the posted `userId` matched the signed user. **Fixed**: the handler now validates `initData` with `validateInitData()` (HMAC-SHA-256 against `BOT_TOKEN`), rejects stale payloads (>24h), and derives `userId` from the signed `user` object — any `userId` in the request body is ignored. Covered by `worker/test/leaderboard-optin.test.js`. | [#182](https://github.com/xlabtg/TON-Bridge/issues/182) — resolved by PR #188. |
...
| R6 | Resolved | `src/_includes/admin-page.njk` + worker | Admin panel surface is gated by Telegram IDs but production data flows depend on the worker APIs being deployed (#121). **Confirmed**: all eight admin views are backed by authorised, mounted worker endpoints (see `ADMIN_PANEL_ENDPOINTS.md`). A smoke check (`worker/tests/adminPanelContract.test.js`, `npm run test:admin-contract`, CI job *Admin Panel Contract Tests*) extracts every `/admin/api/*` call from `assets/js/admin.js` and asserts each is routed, auth-gated (401/403), mounted in `index.js`, and CSP-reachable — so it cannot regress on deploy. | [#186](https://github.com/xlabtg/TON-Bridge/issues/186) — resolved by PR #192. |
